...
generate_data_from_memberships <- function(cl, mu1, mu2, Sig2) {
  library(mvtnorm)
  
  n <- nrow(cl)
  list(rmvt(n, sigma = Sig2, df = 10) + t(mu1)[cl[, 1], ], 
       rmvt(n, sigma=Sig2, df=10) + t(mu2)[cl[, 2], ])
}
...
# of sample size n 
generate_t_data = function(n, pi, mu1, mu2, Sig) { 
...
do_one <- function(delta, n) { 
  ...
  Sig2 <- rbind(c(1.5^2, 0.5), c(0.5, 1.5^2))
  
  # Simulate dataset
  dat <- generate_t_data(n, Pi, mu1, mu2, Sig2)
  x <- dat$x 
  cl <- dat$cl
...
save(results, file=paste("dense-t-results-n", n, "-delta", delta, 
                           "-seed", seed, ".Rdata", sep="")) 
