...
generate_data_from_memberships <- function(cl, mu1, mu2, Sig1, Sig2) {
  n <- nrow(cl)
  p1 <- nrow(mu1)
  p2 <- nrow(mu2)
  list(matrix(rnorm(n * p1), n, p1)%*%Sig1 + t(mu1)[cl[, 1], ],
       matrix(rnorm(n * p2), n, p2)%*%Sig2 + t(mu2)[cl[, 2], ])
}
...
# of sample size n 
generate_Gaussian_data = function(n, pi, mu1, mu2, Sig1, Sig2) { 
  cl <- generate_memberships(n, pi)
  x <- generate_data_from_memberships(cl, mu1, mu2, Sig1, Sig2)
...
do_one <- function(delta, n) { 
  ...
  # Var mat = 1.44 & 0.5 // 0.5 & 1.44
  Sig1 <- chol(rbind(c(1.5^2, 0.5), c(0.5, 1.5^2)))
  Sig2 <- diag(c(1.5^2, 2^2))
  
  # Simulate dataset
  dat <- generate_Gaussian_data(n, Pi, mu1, mu2, Sig1, Sig2)
  x <- dat$x 
  cl <- dat$cl
  
  results <- rep(NA, 4)
  invisible(capture.output(results[1] <- tryCatch(test_indep_clust(x, K1=3, K2=3,  
                                                                   model1="EEE", model2="EEI", init1="EEE", init2="EEE")$pval, 
  ...
  invisible(capture.output(results[3] <- tryCatch(meila_mclust(x, 
                                                               K1=3, K2=3, model1="EEE", model2="EEI", init1="EEE", init2="EEE")[3], 
                                                  error=function(cond) { return(NA) } ) ))
  
  invisible(capture.output(results[4] <- tryCatch(meila_mclust(x, model1="EEE", model2="EEI", init1="EEE", init2="EEE")[3], 
                                                  error=function(cond) { return(NA) } ) ))
...
save(results, file=paste("dense-diagonal-gaussian-results-n", n, "-delta", delta, 
                           "-seed", seed, ".Rdata", sep="")) 
